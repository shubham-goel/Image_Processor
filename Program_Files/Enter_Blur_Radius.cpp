...
void copy_string3(char s1[],Glib::ustring s2){
    int i=0;
    for(;s2[i]!='\0';)
    {
        if(i==14){break;}
...
Enter_Blur_Radius::Enter_Blur_Radius()
{
    //ctor
    set_position(Gtk::WIN_POS_CENTER);
    FILE*blur_coordinates=fopen("Program_Files/Temp_Files/blur_coordinates","wb+");
    float x1=-1;
    if(blur_coordinates!=NULL){
        fwrite(&x1,sizeof(x1),1,blur_coordinates);
    ...
    Gtk::Button *bblur = Gtk::manage(new Gtk::Button("Blur"));
    bblur->signal_clicked().connect(sigc::bind<Gtk::Entry*>(sigc::mem_fun(*this, &Enter_Blur_Radius::on_blur_click), e_x1));
    grid->attach(*bblur, 2, 2, 1, 1);


    vbox->show_all();
}

void Enter_Blur_Radius::on_blur_click(Gtk::Entry *e_x1){
    float x1=0;
    char s_x1[15];
    copy_string3(s_x1,e_x1->get_text());
    if(!is_number3(s_x1)){
        ...
        if((x1<0||x1>20)){dialog("Please Enter a Valid Number between 0 and 20");}
            ...
            FILE*stretch_coordinates=fopen("Program_Files/Temp_Files/blur_coordinates","wb+");
            if(stretch_coordinates==NULL){dialog("Sorry! Unable to Blur Image.");hide();return;}
            else{
...
void Enter_Blur_Radius::dialog(Glib::ustring msg)
{
