...
void copy_string(char s1[],Glib::ustring s2){
    int i=0;
    for(;s2[i]!='\0';)
    {
        if(i==14){break;}
...
void swap_numbers(float&a,float&b){
    float temp=a;
    a=b;
    b=temp;
    return;
...
Enter_Crop_Coordinates::Enter_Crop_Coordinates()
{
    //ctor

    float x1=-1,y1=-1,x2=-1,y2=-1;
    FILE*crop_coordinates=fopen("Program_Files/Temp_Files/Crop_Coordinates","wb+");
    if(crop_coordinates==NULL){dialog("Sorry! Unable to Crop Image.");}
    else{
        fwrite(&x1,sizeof(x1),1,crop_coordinates);
        fwrite(&y1,sizeof(y1),1,crop_coordinates);
        fwrite(&x2,sizeof(x2),1,crop_coordinates);
        fwrite(&y2,sizeof(y2),1,crop_coordinates);
        fclose(crop_coordinates);
    }

    set_default_size(400, 200);
    set_title("Enter Crop Coordinates");
    ...
    Gtk::Label *l_2 = Gtk::manage(new Gtk::Label("Enter Coordinates of Bottom-Right Corner  "));
    ...
    Gtk::Button *bcrop = Gtk::manage(new Gtk::Button("Crop"));
    bcrop->signal_clicked().connect(sigc::bind<Gtk::Entry*, Gtk::Entry*, Gtk::Entry*, Gtk::Entry* >(sigc::mem_fun(*this, &Enter_Crop_Coordinates::on_crop_click), e_x1,e_y1,e_x2,e_y2));
//    bcrop->signal_clicked().connect(sigc::mem_fun(*this, &Enter_Crop_Coordinates::on_crop_click));
    grid->attach(*bcrop, 3, 4, 1, 1);


    vbox->show_all();
}

void Enter_Crop_Coordinates::on_crop_click(Gtk::Entry *e_x1,Gtk::Entry *e_y1,Gtk::Entry *e_x2,Gtk::Entry *e_y2){
    float x1=0,y1=0,x2=0,y2=0;
    char s_x1[15],s_y1[15],s_x2[15],s_y2[15];
    copy_string(s_x1,e_x1->get_text());
    copy_string(s_x2,e_x2->get_text());
    copy_string(s_y1,e_y1->get_text());
    copy_string(s_y2,e_y2->get_text());
    if(!is_number(s_x1)||!is_number(s_x2)||!is_number(s_y1)||!is_number(s_y2)){
        ...
        if((x1<0||x1>100)||(y1<0||y1>100)||(x2<0||x2>100)||(y2<0||y2>100)||x2<=x1||y2>=y1){dialog("Please Enter Valid Numbers!");}
        else{
...
void Enter_Crop_Coordinates::dialog(Glib::ustring msg)
{
